...
def chpic_stickers_grubber():
    ...
    handler1 = logging.FileHandler(f"logs/cron/chpic.log", mode='w')
    ...
    try:
        with open("cron/stickers/files/stickers_chpic.json", "w", encoding="utf-8") as f:
...
def combot_stickers_grubber():
    ...
    handler2 = logging.FileHandler('logs/cron/combot.log', mode='w')
    ...
    try:
        with open("cron/stickers/files/stickers_combot.json", "w", encoding="utf-8") as f:
...
def tlgrm_stickers_grubber():
    ...
    handler3 = logging.FileHandler(f"logs/cron/tlgrm.log", mode='w')
    ...
    try:
        with open("cron/stickers/files/stickers_tlgrm.json", "w", encoding="utf-8") as f:
            json.dump(results, f, indent=4, ensure_ascii=False)
        final_time = time.time() - begin
        logger_tlgrm.info(
            f'Закончил с chpic сайтом. Заняло времени - {final_time}. Всего страниц обработано - {page_count}')
...
def main():
    ...
    handler = logging.FileHandler(f"/home/andrey/python/bot3.0/logs/cron/main.log", mode='w')
    ...
    try:
        ...
        try:
            with open("stickers_total.files", "w", encoding="utf-8") as f:
...
#     with open("old_json/stickers_chpic.json", "r", encoding="utf-8") as file:
#         chpic_list = json.load(file)
#     with open("old_json/stickers_combot.json", "r", encoding="utf-8") as file:
#         combot_list = json.load(file)
#     with open("old_json/stickers_tlgrm.json", "r", encoding="utf-8") as file:
#         tlgrm_list = json.load(file)
#
